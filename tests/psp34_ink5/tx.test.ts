...
describe('MY_PSP34 (ink5)', () => {
  ...
  it('Returns total supply', async () => {
    ...
    const resultBefore = (await query.totalSupply()).value.ok?.toNumber();

    await tx.mint(UserAlice.address, IdBuilder.U8(new BN(resultBefore!.valueOf() + 1)));

    const resultAfter = (await query.totalSupply()).value.ok?.toNumber();
  ...
  it('Can mint any Id', async () => {
    ...
    const totalSupplyAfter = (await query.totalSupply()).value.ok?.toNumber();
