...
import ContractAbi from '../artifacts/vester.json';
import QueryMethods from '../query/vester';
import BuildExtrinsicMethods from '../build-extrinsic/vester';
import TxSignAndSendMethods from '../tx-sign-and-send/vester';
import MixedMethods from '../mixed-methods/vester';
import EventsClass from '../events/vester';
import EVENT_DATA_TYPE_DESCRIPTIONS from '../event-data/vester.json';
import type { EventDataTypeDescriptions } from '../shared/types';

export default class VesterContract {
   ...
   * @example
   * ```typescript
   * const contract = new VesterContract(address, signerAlice, api);
   * await contract.mint(signerBob.address, 100);
   * await contract.withSigner(signerBob).transfer(signerAlice.address, 100);
   * ```
   */
  withSigner(signer: KeyringPair): VesterContract {
    return new VesterContract(this.address, signer, this.nativeAPI);
  }
   ...
   */
  withAddress(address: string): VesterContract {
    return new VesterContract(address, this.signer, this.nativeAPI);
  }
   ...
   */
  withAPI(api: ApiPromise): VesterContract {
    return new VesterContract(this.address, this.signer, api);
  }
}
