...
import MyPsp22 from '../contracts/my_psp22';
...
const fileName = 'my_psp22';

export default class MyPsp22Deployer {
   ...
   */
  async new(initialSupply: string | number | BN, __options?: ContractOptions) {
    const codePromise = this.createCodePromise();

    const gasLimit = (await genValidContractOptionsWithValue(this.nativeAPI, __options)).gasLimit as WeightV2 as any;

    const storageDepositLimit = __options?.storageDepositLimit;
    const tx = codePromise.tx['new']!({ gasLimit, storageDepositLimit, value: __options?.value }, initialSupply);
    let response;
    ...
    const contractObj = getContractObjectWrapper(this.nativeAPI, MyPsp22, address, this.signer);
    return {
