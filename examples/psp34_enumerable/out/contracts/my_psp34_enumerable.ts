...
import ContractAbi from '../artifacts/my_psp34_enumerable.json';
...
export default class MyPsp34EnumerableContract {
   ...
   * @example
   * ```typescript
   * const contract = new MyPsp34EnumerableContract(address, signerAlice, api);
   * await contract.mint(signerBob.address, 100);
   * await contract.withSigner(signerBob).transfer(signerAlice.address, 100);
   * ```
   */
  withSigner(signer: KeyringPair): MyPsp34EnumerableContract {
    return new MyPsp34EnumerableContract(this.address, signer, this.nativeAPI);
  }
   ...
   */
  withAddress(address: string): MyPsp34EnumerableContract {
    return new MyPsp34EnumerableContract(address, this.signer, this.nativeAPI);
  }
   ...
   */
  withAPI(api: ApiPromise): MyPsp34EnumerableContract {
    return new MyPsp34EnumerableContract(this.address, this.signer, api);
  }
}
