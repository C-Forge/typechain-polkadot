...
export default class MyPsp22Deployer {
   ...
   */
  async new(totalSupply: string | number | BN, __options?: ContractOptions) {
    const codePromise = this.createCodePromise();

    const gasLimit = (await genValidContractOptionsWithValue(this.nativeAPI, __options)).gasLimit as WeightV2 as any;

    const storageDepositLimit = __options?.storageDepositLimit;
    const tx = codePromise.tx['new']!({ gasLimit, storageDepositLimit, value: __options?.value }, totalSupply);
    let response;
