...
import ContractAbi from '../artifacts/psp22.json';
...
import EVENT_DATA_TYPE_DESCRIPTIONS from '../event-data/psp22.json';
import type { EventDataTypeDescriptions } from '../shared/types';

export default class Psp22Contract {
   ...
   * @example
   * ```typescript
   * const contract = new Psp22Contract(address, signerAlice, api);
   * await contract.mint(signerBob.address, 100);
   * await contract.withSigner(signerBob).transfer(signerAlice.address, 100);
   * ```
   */
  withSigner(signer: KeyringPair): Psp22Contract {
    return new Psp22Contract(this.address, signer, this.nativeAPI);
  }
   ...
   */
  withAddress(address: string): Psp22Contract {
    return new Psp22Contract(address, this.signer, this.nativeAPI);
  }
   ...
   */
  withAPI(api: ApiPromise): Psp22Contract {
    return new Psp22Contract(this.address, this.signer, api);
  }
}
