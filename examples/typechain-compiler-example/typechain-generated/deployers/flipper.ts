...
export default class FlipperDeployer {
   ...
   */
  async new(initValue: boolean, __options?: ContractOptions) {
    const codePromise = this.createCodePromise();

    const gasLimit = (await genValidContractOptionsWithValue(this.nativeAPI, __options)).gasLimit as WeightV2 as any;

    const storageDepositLimit = __options?.storageDepositLimit;
    const tx = codePromise.tx['new']!({ gasLimit, storageDepositLimit, value: __options?.value }, initValue);
    let response;
    ...
    const contractObj = getContractObjectWrapper(this.nativeAPI, Flipper, address, this.signer);
    return {
   ...
   *
   */
  async default(__options?: ContractOptions) {
    const codePromise = this.createCodePromise();

    const gasLimit = (await genValidContractOptionsWithValue(this.nativeAPI, __options)).gasLimit as WeightV2 as any;

    const storageDepositLimit = __options?.storageDepositLimit;
    const tx = codePromise.tx['default']!({ gasLimit, storageDepositLimit, value: __options?.value });
    let response;
